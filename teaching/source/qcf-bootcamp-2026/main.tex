...
for $\pub{s}$ -- the one unknown left -- and publish it. Only someone who knows $\priv{d}$ can solve it. \pause
...
\m \textbf{Why it balances}: $\pub{\beta}^{\pub{r}} = \pub{\alpha}^{\priv{d}\pub{r}}$, and the rule $\alpha^{a}\alpha^{b}=\alpha^{a+b}$ still holds on the clock, so the left side is $\pub{\alpha}^{\priv{d}\pub{r} + \priv{k}\pub{s}} = \pub{\alpha}^{\pub{x}}$. 
