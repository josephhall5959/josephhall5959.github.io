...
\m{The rate has no gradient in credit score or income. Default rates fall fifty-fold across the same score deciles. The heterogeneity in default is exposure to the default margin.}
...
\begin{frame}{Three provisos, three results}
\begin{enumerate}
...
\item \textbf{The contract's length.} A longer contract at a fixed payment adds far payments, raises a balance-scaled penalty, adds months of exposure, and across borrowers changes who selects which length. A rate comes from it only with the borrower held fixed and the penalty channel decayed.
\end{enumerate}
...
\centering\includegraphics[width=0.8\textwidth,height=0.6\textheight,keepaspectratio]{figures/fig_rho_headline.pdf}
\begin{itemize}\small
...
\centering\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{figures/fig_horizon_power.pdf}
\begin{itemize}\small
...
\centering\includegraphics[width=0.9\textwidth,height=0.6\textheight,keepaspectratio]{figures/fig_rho_within.pdf}
\begin{itemize}\small
...
\m{No gradient in score or income, a hump in the middle deciles, and sets that include zero in the two top deciles, while default rates fall fifty-fold across the same deciles.}
\end{itemize}
\end{frame}

\begin{frame}{What the bureau adds at population scale}
\centering\includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{figures/fig_exposure_groups.pdf}
\begin{itemize}\small
...
\input{slide_laboratories}

\input{slide_ladder}

\begin{frame}{The run-off of the payment margin validates the stopping model}
\centering\includegraphics[width=0.9\textwidth,height=0.55\textheight,keepaspectratio]{figures/fig_runoff_profiles.pdf}
\begin{itemize}\small
...
\begin{frame}{The rate does not differ across groups}
\centering\resizebox{0.72\textwidth}{!}{\input{tables/tab_rho_within_compact}}
\begin{itemize}\small
...
\m{Heterogeneity in default is in the level of the margin, exposure, and not in how the horizon is discounted.}
\end{itemize}
\end{frame}

\begin{frame}{The rate by subgroup, from the one experiment that reports both arms by subgroup}
\centering\includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{figures/fig_rho_subgroups.pdf}
\begin{itemize}\small
...
\centering\includegraphics[width=\textwidth,height=0.58\textheight,keepaspectratio]{figures/fig_het_agreement.pdf}
\begin{itemize}\small
...
\centering\includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{figures/fig_geo_correlates.pdf}
\begin{itemize}\small
\m{Across 91 zip-code areas exposure runs from $0.33$ to $0.67$ between the tenth and ninetieth percentiles, as dispersed as across income quartiles. One standard deviation of income per return raises it by $0.07$ across states, and one of the Republican share lowers it by $0.06$.}
\m{With the place's score, income and contract-length composition held fixed, the vote's gradient is zero and income's survives only across states. The political map of default is an income map.}
\end{itemize}
\end{frame}
...
\m{\textbf{The rate.} $0.11$ over the horizon of a contract, from two loans of the same borrower, with no gradient in score or income and a gradient in age. The published experiments accept it and, run over windows of years at horizons of four to twenty-six years, fix its sign and not its level.}
...
\centering\includegraphics[width=0.9\textwidth,height=0.62\textheight,keepaspectratio]{figures/fig_ledger_paths.pdf}
...
\centering\includegraphics[width=0.75\textwidth,height=0.62\textheight,keepaspectratio]{figures/fig_ledger_rho.pdf}
...
\centering\includegraphics[width=0.7\textwidth,height=0.4\textheight,keepaspectratio]{figures/fig_ledger_near.pdf}\\
...
\centering\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]{figures/fig_stability.pdf}
...
\centering\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{figures/fig_spec.pdf}
...
\centering\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]{figures/fig_gap.pdf}\backto
\end{frame}

\begin{frame}{The term margin has two channels}
\centering\includegraphics[width=0.8\textwidth,height=0.55\textheight,keepaspectratio]{figures/fig_termmargin.pdf}
...
\centering\includegraphics[width=0.48\textwidth,height=0.5\textheight,keepaspectratio]{figures/fig_place.pdf}\hfill\includegraphics[width=0.48\textwidth,height=0.5\textheight,keepaspectratio]{figures/fig_chargeoff.pdf}
...
\centering\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{figures/fig_exposure_geo.pdf}\backto
\end{frame}
