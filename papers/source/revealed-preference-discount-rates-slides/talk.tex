...
\centering\resizebox{0.68\textwidth}{!}{\input{tables/tab_rho_within_compact}}
\begin{itemize}\small
...
\m{With the place's score, income and contract-length composition held fixed, the vote's gradient is zero and income's survives only across states. The political map of default is an income map, and not a map of the rate, whose ratio cancels the level of default by construction.}
\end{itemize}
\end{frame}
...
\m{A rate by place. The state ratio net of collection law: recourse explains much of the dispersion with the sign the penalty channel does not predict, and 28 of 50 states stay outside the formula's range.}
