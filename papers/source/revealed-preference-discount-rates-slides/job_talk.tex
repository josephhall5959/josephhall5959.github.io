...
\begin{wideitemize}
  ...
  \item Heterogeneous agents: aggregation theory for what a population
        ...
        \ph{aggregation simulation figure}
  \item Marginal-utility correction: \alert{measured, not assumed} ---
...
\begin{frame}[label=power]{Power is a theorem here, not a hope}
\begin{center}
\Large
...
\begin{wideitemize}
  ...
  \item Win condition: IV and FE agree quantitatively where both run,
        apples to apples --- then FE carries the heterogeneity
        \ph{lender-FE iteration --- final table in flight}
\end{wideitemize}
...
\begin{wideitemize}
  \item \alert{65\% of 2.46M auto defaulters keep paying other creditors}
        ...
        average; only 1.6\% default on everything) --- a portfolio choice,
        not a collapse
...
\begin{frame}{By credit score: monotone, precise, and modest}
\begin{wideitemize}
  \item Varying-coefficient design: the two logits carry heterogeneity in
        their derivatives, so every default informs every cell ---
        standard errors of \alert{0.4pp} where cell-splitting gives multiples
  \item Three-year rates decline monotonically across score deciles:
        \alert{26.7\% (d1) $\to$ 23.2\% (d10)}
  \item The gradient is real but \emph{modest} --- high default rates among
        low-score borrowers are mostly not impatience
\end{wideitemize}
\end{frame}

\begin{frame}{By income: the patient rich, measured}
\begin{wideitemize}
  \item Same design, income quartiles (at median score):
        \alert{25.8\% $\to$ 21.8\%} from the bottom to the top quartile
  \item Individual-level gradient, holding score and region ---
        distinct from (and cleaner than) the zip-level income correlate,
        which dies under the selection control
  \item Correction invariance: a common $\lambda$ cannot generate
        cross-group gradients
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
  ...
  \item \ph{cluster map figure --- choropleth by $\widehat{\beta}$}
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
  ...
  \item Income level: raw positive, \alert{dies under the selection control}
        --- reported as a selection artifact, not a finding
...
\begin{wideitemize}
  \item Heterogeneous-agents aggregation: measured aggregates load on the
        ...
        answer different questions \ph{aggregation simulation figure}
  \item The 25\% median coexists with standard macro calibrations ---
        and the paper shows exactly how
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
  \item Incidence: debt relief and forbearance are transfers \emph{to} the
        impatient margin --- measured, not assumed
...
\ph{two-period derivation, then $T$-period envelope argument}
\end{frame}

\begin{frame}{Backup: continuous-time statement}
\ph{optimal-stopping formulation and the identity as smooth-pasting}
\end{frame}
...
\begin{frame}[label=b_tvr]{Backup: is the clock doing the work? No.}
\begin{wideitemize}
...
\ph{correlates with and without default-share control; score-band-within-
cluster estimates when they land}
...
\begin{wideitemize}
  ...
  \item The pre-registered fix: lender FE, identification from
        \emph{menu changes within lender} \ph{final agreement table}
\end{wideitemize}
