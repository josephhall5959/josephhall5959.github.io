...
\label{tab:rho_place}
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/tab_rho_place}}
\vspace{0.5em}
...
Across states, holding the score tercile fixed, the cell ratio is a regional object (\texttt{results/place\_cells\_correlates.csv}). Census-region effects explain 52 percent of its variance across the 129 cells with covariates; the 2020 Republican share alone explains 39 percent, $+0.28$ per standard deviation (standard error $0.03$), income per return $-0.23$, and the unemployment rate $-0.22$; the 24 cells with a negative ratio at 24 months are in Alaska, Arizona, California, Connecticut, Delaware, Hawaii, Minnesota, Nevada, North Dakota, Oregon and Washington, and the cells above the ceiling are in the South. Jointly with income, the default rate, contract length and the post-repossession window, the vote survives ($+0.27$ per standard deviation, standard error $0.06$) and income does not; the auto collection rules add nothing to the cell level once the region is in. Within a score tercile and within borrower, then, the term response is highest where the vote is Republican and lowest on the West Coast, in both cases beyond what the preference channel allows in that direction, so the correlation is not a statement about the far payments' weight until the channel that moves the term response beyond its bound is separated from it.
...
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/tab_rho_place_states}}
\vspace{0.5em}
\begin{minipage}{0.95\textwidth}
